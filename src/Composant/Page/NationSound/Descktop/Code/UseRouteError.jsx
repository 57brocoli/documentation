...
function UseRouteError() {
    ...
    if (data.section) {
        var dataExtract = data.section.filter(section => section.titre === 'UseRouterError')
        var useRouteError = dataExtract[0]
    }
        ...
        <main className='background pt-20 px-10'>
            ...
            {useRouteError &&
                <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration: 0.2}}>
                    <h2 className='text-2xl font-bold mb-10 orange'>{useRouteError.titre}</h2>
                    {useRouteError.content.introduction.map((intro,index)=>{
                        return(
                            <p key={index} className='text-lg font-semibold'>{intro}</p>
                        )
                    })}
                    <p>Import :</p>
                    <img src={useRouteError.content.imgImport} className='rounded border-2 my-2 h-10' alt="import du fichier" />
                    <hr className='my-7'/>
                    <article>
                        <h2 className='text-xl font-bold'>Cas d'utilisation :</h2>
                        {useRouteError.content.utilisation.map((cas, index)=>{
                                return(
